resource "aws_route53_record" "dns-record" {
  ...
  name    = "${var.COMPONENT}-dev.${data.terraform_remote_state.vpc.outputs.PRIVATE_HOSTEDZONE_NAME}" : "${var.COMPONENT}-dev.${data.terraform_remote_state.vpc.outputs.PUBLIC_HOSTEDZONE_NAME}"
  type    = "CNAME"
  ttl     = 10
